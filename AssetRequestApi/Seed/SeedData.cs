...
namespace AssetRequestApi.Seed;
...
public static class SeedData
{
    ...
    private static async Task SeedAppSettingsAsync(ApplicationDbContext context)
    {
        var defaults = new[]
        {
            ...
            new AppSetting { Key = "Palette.Plot.PendingReview", Value = "#E879F9", ValueType = "Color", Description = "Map colour for pending review plots." },
    ...
    private static async Task SeedPlotsAsync(ApplicationDbContext context, IWebHostEnvironment environment)
    {
        ...
        foreach (var item in seedItems.Where(item => !string.IsNullOrWhiteSpace(item.Code)))
        {
            ...
            {
                boundary = new WKTReader().Read(item.BoundaryWkt);
                boundary.SRID = 0;
            }
    ...
    private static string ResolvePlotSeedPath(IWebHostEnvironment environment)
    {
